...
##---------------------------------------------------------------------------------------------------
#PBS -l select=16:ncpus=1:host=trnckun11
##---------------------------------------------------------------------------------------------------
...
export PATH=/home/880212l26111201/qe_code/qe-6.8/bin:$PATH
echo -e "using MPI: mpiexec is \n$(type -p mpiexec)"
#np=$(cat $PBS_NODEFILE |wc -l)

for i in {1..50}; do

run="mpiexec pw.x -in structure_$i.pwi > structure_$i.pwo"  # type waht you mainly want to run here
eval $run 1> >(tee ${PBS_JOBID}.${PBS_JOBNAME}.out) 2> >(tee -a ${PBS_JOBID}.${PBS_JOBNAME}.out)

echo "structure_$i done"

done

#cp structure_*.pwo /home/880212l26111201/sscha/test/data/output_espresso/

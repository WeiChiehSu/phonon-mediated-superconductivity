...
\Downarrow \\
\text{3.KS-eq:} \left[ -\nabla^2 + V(r) + V_{\text{Hatree}}(r)+V_{xc}(r)  \right] \Psi_{i} (r) = \varepsilon \Psi_{i}(r) \\
\Downarrow \\
4.n_{\text{new}}(r) = \sum_{i}^{}   \left| \Psi_{i}(r) \right|^2 \\
\Downarrow \\
...
\Downarrow \\
\text{Guess-perturbate-charge-density and perturbate-potential:} \, \Delta n(r);\Delta V_{\text{per}}^{q} (r) \\
\Downarrow \\
\text{The total perturbate-potential :} \,\Delta V_{total}  (r) = \Delta V_{\text{per}}^{q} (r) + e^2 \int \frac{\Delta n(r)}{|r - r'|} dr \\
\Downarrow \\
\text{Fourier-Transformation:} \, \Delta V_{total}  (r) = -\frac{1}{\sqrt{N_{p}}} \sum_{k} e^{-i(q+G)\cdot r} \Delta V_{total}(q+G) \\
\Downarrow \\
\Delta n_{\text{new}}(q+G) = \frac{4}{N \Omega} \sum_{k} \sum_{c,v} \frac{\langle \Psi_{v,k} | e^{-i(q+G)\cdot r} | \Psi_{c,k+q} \rangle \langle \Psi_{c,k+q} | \Delta V_{total}(q+G) | \Psi_{v,k} \rangle}{\varepsilon_{v,k} - \varepsilon_{c,k+q}} \\
\Downarrow \\

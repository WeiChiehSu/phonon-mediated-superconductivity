...
\Downarrow \\
\text{Fourier-Transformation:} \, \Delta V_{total}  (r) = -\frac{1}{\sqrt{N_{p}}} \sum_{k} e^{-i(q+G)\cdot r} \Delta V(q+G) \\
\Downarrow \\
\Delta n_{\text{new}}(q+G) = \frac{4}{N \Omega} \sum_{k} \sum_{c,v} \frac{\langle \Psi_{v,k} | e^{-i(q+G)\cdot r} | \Psi_{c,k+q} \rangle \langle \Psi_{c,k+q} | \Delta V(q+G) | \Psi_{v,k} \rangle}{\varepsilon_{v,k} - \varepsilon_{c,k+q}} \\
\Downarrow \\

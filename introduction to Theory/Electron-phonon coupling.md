...
$$
第二項代表的物理意義可以從位能和微分的基本意義出發:-V_{K-S}(r_{i}-R_{j})代表電子從原子核位置R_{j} 移動到r_{i}時庫侖力做的功,對R_{j}微分表明當原子核位置微小變化時,電子和原子核間的庫倫力做功變化,從力學的角度來說,當原本固定的位能產生變化時,表示兩者間受到了一股額外的吸引力,現在\frac{\partial V_{K-S}(r_{i}-R_{j})}{\partial R_{j} }乘上原子核的位移r_{j}後,為系統因這股額外的力產生的Energy變化,與就是Hamilton的修正項.，對應「電子-聲子耦合」（electron-phonon coupling）:
$$

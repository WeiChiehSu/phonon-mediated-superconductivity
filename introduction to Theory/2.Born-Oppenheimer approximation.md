...
$$
\phi (r_{i} ,R_{a})代表原子核偏離平衡點R_{a}時的電子wave{}function,而\chi (R_{a})代表原子核運動的wave{}function. 將 \phi (r_{i} ,R_{a})\chi (R_{a}) 代入 \widehat{H}\Psi (r_{i} ;R_{a} ) 中，解 Schrödinger{}equation:
$$
...
$$
這項代表電子處於R_{a}的動能,由於電子和原子核的質量差距非常大,因此\frac{m_{electron}}{m_{a} }的值約為電子動能的10^{-4}  至 10^{-5} 倍,所以這項也能略去.
$$
...
$$
可以得知,總波函數中的電子運動和核運動的項可以有效分開,先求解固定R_{a}的電子eigenvalue-\varepsilon _{n}(R_{a})(只考慮電子的wave{}function,不須考慮核的wave{}function)作為參照,再來求解eigenvalue.
$$
